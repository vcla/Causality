# THIS ONE IS THE FIRST IN A WHILE TO ACTUALLY HAVE PRECISION AND RECALL
# HOWEVER, BECAUSE OF A DEGREES OF FREEDOM ISSUE IN HOW WE'RE SPLITTING UP
# POSSIBLE HITS, AND THE SPARSE DATA PER VIDEO CLIP, WHEN CALCULATING PRECISION
# AND RECALL WITHIN A SINGLE CLIP, PRECISION AND RECALL ARE OFTEN THE SAME
# 2015-11-05; next version will sum FN/FP/TP/TN across all clips, then calculate
# SEE analyzeData-nearesthuman-pr-noreally.py! :)
	...
	if filename.endswith(".csv"):
		with open(os.path.join(kCSVDir,filename),"r") as csv:
			try:
				...
				clip_scores = defaultdict(lambda: defaultdict(lambda: {'TP':0, 'FP':0, 'TN':0, 'FN':0, 'precision': 0., 'recall': 0., 'f1score': 0.}))
				for computerType in kComputerTypes:
					...
					TP = clip_scores[computerType][field_group[0]]["TP"]
					FP = clip_scores[computerType][field_group[0]]["FP"]
					TN = clip_scores[computerType][field_group[0]]["TN"]
					FN = clip_scores[computerType][field_group[0]]["FN"]
					try:
						clip_scores[computerType][field_group[0]]["precision"] = float(TP) / (TP + FP)
					except ZeroDivisionError:
						pass
					try:
						clip_scores[computerType][field_group[0]]["recall"] = float(TP) / (TP + FN)
					except ZeroDivisionError:
						pass
					try:
						clip_scores[computerType][field_group[0]]["f1score"] = 2. * TP / (2 * TP + FP + FN)
					except ZeroDivisionError:
						pass
...
prefix_hit_precision_sum = defaultdict(lambda: defaultdict(float))
prefix_hit_recall_sum = defaultdict(lambda: defaultdict(float))
prefix_hit_f1score_sum = defaultdict(lambda: defaultdict(float))
prefix_hitN = defaultdict(lambda: defaultdict(int))
prefix_hit_precision = defaultdict(lambda: defaultdict(float))
prefix_hit_recall = defaultdict(lambda: defaultdict(float))
prefix_hit_f1score = defaultdict(lambda: defaultdict(float))
for filename in overall_scores:
	for computer in overall_scores[filename]:
		...
		for prefix in overall_scores[filename][computer]:
			prefix_hit_precision_sum[prefix][computer] += overall_scores[filename][computer][prefix]['precision']
			prefix_hit_recall_sum[prefix][computer] += overall_scores[filename][computer][prefix]['recall']
			prefix_hit_f1score_sum[prefix][computer] += overall_scores[filename][computer][prefix]['f1score']
			prefix_hitN[prefix][computer] += 1
for prefix in prefix_hit_recall_sum:
	for computer in prefix_hit_recall_sum[prefix]:
		prefix_hit_precision[prefix][computer] = prefix_hit_precision_sum[prefix][computer] / prefix_hitN[prefix][computer]
		prefix_hit_recall[prefix][computer] = prefix_hit_recall_sum[prefix][computer] / prefix_hitN[prefix][computer]
		prefix_hit_f1score[prefix][computer] = prefix_hit_f1score_sum[prefix][computer] / prefix_hitN[prefix][computer]

# now we print out our carefully crafted table :)
print("\t".join(("prefix","N","computer","precision","recall","f1score",)))
summary = defaultdict(lambda: defaultdict(float))
summary_N = defaultdict(int)
sum_fluents = defaultdict(lambda: defaultdict(float))
sum_fluents_N = defaultdict(int)
sum_actions = defaultdict(lambda: defaultdict(float))
sum_actions_N = defaultdict(int)
for prefix in prefix_hit_precision_sum:
	for computer in prefix_hit_precision_sum[prefix]:
		if not args.smart and computer in ["causalsmrt", "origsmrt", ]:
			continue
		precision = prefix_hit_precision[prefix][computer]
		recall = prefix_hit_recall[prefix][computer]
		f1score = prefix_hit_f1score[prefix][computer]
		if not args.summary:
			print("\t".join((prefix, str(prefix_hitN[prefix][computer]), computer, "{:.4f}".format(precision), "{:.4f}".format(recall), "{:.4f}".format(f1score))))
...
for computer in summary:
	...
	print("\t".join(("FLUENTS", str(sum_fluents_N[computer]), computer, "{:.4f}".format(precision), "{:.4f}".format(recall), "{:.4f}".format(f1score))))
...
for computer in summary:
	...
	print("\t".join(("ACTIONS", str(sum_actions_N[computer]), computer, "{:.4f}".format(precision), "{:.4f}".format(recall), "{:.4f}".format(f1score))))
...
for computer in summary:
	...
	print("\t".join(("SUM", str(summary_N[computer]), computer, "{:.4f}".format(precision), "{:.4f}".format(recall), "{:.4f}".format(f1score))))
